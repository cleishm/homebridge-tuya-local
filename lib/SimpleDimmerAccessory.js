...
class SimpleDimmerAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.device.on('change', (changes, state) => {
            if (changes.hasOwnProperty(this.dpPower)) {
                ...
                if (!this._isLocalWrite()) this.log.info(`${this.device.context.name} switched ${changes[this.dpPower] ? 'ON' : 'OFF'} externally — state ${JSON.stringify(state)}`);
    ...
    setPower(value, callback, context, connection) {
        ...
        this.log.info(`${this.device.context.name} switched ${value ? 'ON' : 'OFF'} by ${this._describeConnection(connection)}`);

class BaseAccessory {
    ...
    // Identifies which (child) bridge this accessory instance belongs to, so a
    // device exposed through several bridges can be told apart in the log.
    _logName() {
        const bridge = this.platform.config && this.platform.config._bridge;
        return bridge && bridge.name ? `${this.device.context.name} [${bridge.name}]` : this.device.context.name;
    }

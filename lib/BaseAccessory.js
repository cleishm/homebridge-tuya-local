class BaseAccessory {
    ...
    // Identifies which platform instance this accessory belongs to, so a device
    // exposed twice (e.g. through two child bridges, one per HomeKit home) can
    // be told apart in the log. Homebridge strips _bridge from the config it
    // hands to a child bridge, so this relies on the platform's own name.
    _logName() {
        const platformName = this.platform.config && this.platform.config.name;
        return platformName ? `${this.device.context.name} [${platformName}]` : this.device.context.name;
    }

    // Describes the HomeKit controller behind a characteristic write. The
    // connection is undefined when the write did not come over HAP.
    _describeConnection(connection) {
        if (!connection) return 'homebridge (no HAP connection)';
        return `${connection.remoteAddress}${connection.username ? ` (${connection.username})` : ''}`;
    }
